    ...
    mutate(
      ...
      carbsn1 = ifelse(
        ...
        unlist(struc)[[6]] %>%
          str_remove("[nmdtOP]") %>%
          as.numeric() %>% 
          # easiest way to get rid of a leading dash
          abs(),
        NA
      ),
      dbonsn1 = ifelse(
      ...
      # this will throw a warning for symmetric PLs
      carbsn2 = ifelse(
        "|" %in% unlist(struc),
        unlist(struc) %>% #head(10) %>% print() %>% #TEST
          .[[8]] %>% 
          str_remove("[nmdtOP]") %>%
          as.numeric() %>% 
          # easiest way to get rid of a leading dash
      ...
      # is there MS2-level data?
      #ms2 = ("|" %in% unlist(struc)) | str_detect(id, "L") %>% unlist(),
      ms2 = str_detect(id, "[|L]") %>% unlist(),
      ...
      # quick patch for symmetric PLs
      # really gotta refactor whole thing! 20250630
      # fix the dbonds first
      dbonsn2 = ifelse((tails == 2) & is.na(carbsn2), dbonsn1, dbonsn2),
      # then fill # carbons
      carbsn2 = ifelse((tails == 2) & is.na(carbsn2), carbsn1, carbsn2)
    )
}
...
# High-contrast scheme for the label text on a bar plot
chroma_tx = chroma_cl %>% 
  DescTools::ColToGray() %>% # for perceptual uniformity
  setNames(names(chroma_cl)) %>% 
  DescTools::ColToHsv() %>% 
  .['v',] %>% 
  lapply(., function(x)(ifelse(x >= 0.5, "black", "white"))) %>% 
  unlist()

# Scheme for outlining light-colored lipids in light mode
chroma_ol_lite = chroma_cl %>% 
  DescTools::ColToGray() %>% # for perceptual uniformity
  setNames(names(chroma_cl)) %>% 
  DescTools::ColToHsv() %>% 
  .['v',] %>% 
  lapply(., function(x)(ifelse(x >= 0.95, "black", "white"))) %>% 
  unlist() %>% as.list()

# In dark mode, put white outlines on really dark colors
chroma_ol_dark = chroma_cl %>% 
  DescTools::ColToGray() %>% # for perceptual uniformity
  setNames(names(chroma_cl)) %>% 
  DescTools::ColToHsv() %>% 
  .['v',] %>% 
  lapply(., function(x)(ifelse(x <= 0.50, "white", "black"))) %>% 
  unlist() %>% as.list()
